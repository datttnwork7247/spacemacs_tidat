...
-spec do_init_node(ProjectName    :: string(),
                   ...
                   ErlangCookie   :: string(),
                   new_node | existing_node) -> ok | {error, term()}.
%%------------------------------------------------------------------------------
do_init_node(ProjectName,
    ...
    try
        case ErlangCookie of
            undefined -> ok;
            _         -> ok = edts_dist:set_cookie(Node,
                                                   list_to_atom(ErlangCookie))
        end,
        RemoteServices =
            case NodeInitializationState of
                new_node ->
                    ...
                    edts_plugins:specs(),
                    ok = lists:foreach(fun(Spec) ->
                                               edts_dist:load_app(Node, Spec)
                                       end,
                                       edts_plugins:specs()),
                    PluginRemoteLoad =
                    ...
                    ?LOG_DEBUG("Loading modules on ~p: ~p",
                               [Node, ModulesToLoad]),
